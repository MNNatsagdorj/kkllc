    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(4,10,20,.66)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }}>
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 3 }}>
          {order.driver_id ? 'Жолооч солих' : 'Жолооч хуваарилах'} — <span className="mono" style={{ color: 'var(--kraft)' }}>{fmtOrderNo(order.id)}</span>
        </div>
        <div className="mono" style={{ fontSize: 12, color: 'var(--mut)', marginBottom: 14 }}>
          ...
          {order.driver && <> · одоо: <b style={{ color: 'var(--kraft)' }}>{order.driver.name}</b></>}
        </div>
        ...
        {warning ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {drivers.map((d) => {
              const isCurrent = d.id === order.driver_id;
              return (
                <button key={d.id} onClick={() => !isCurrent && pickDriver(d)} disabled={busy || isCurrent}
                  style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', borderRadius: 10, border: isCurrent ? '1px solid color-mix(in srgb, var(--kraft) 55%, transparent)' : '1px solid var(--line)', background: 'var(--ink3)', color: '#EFECE3', cursor: isCurrent ? 'default' : 'pointer', textAlign: 'left', opacity: isCurrent ? 0.75 : 1 }}>
                  ...
                  {isCurrent && (
                    <span className="st-chip" style={{ '--st': 'var(--kraft)' } as React.CSSProperties}>одоо</span>
                  )}
                </button>
              );
            })}
